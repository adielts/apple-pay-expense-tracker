...
| Key | Type | Value |
|-----|------|-------|
| `date` | Text | `2026-02-23` |
| `time` | Text | `16:00` |
| `merchant` | Text | `קפה טסט` |
| `amount` | Number | `50` |
| `category` | Text | `🍔 מזון` |
| `card` | Text | `מזומן` |
...
2. בחר **📊 סיכום חודשי** → אמור להופיע:
   ...
   ═══════════════════════════
   💰 סה"כ: ₪50
   2026-02-23 | קפה טסט | ₪50 | 🍔 מזון | ****מזומן
   ```
...
3. כדי לטסט לימוד: הוסף ידנית ב-Data Jar:
   ...
   - Key: `קפה טסט`
   - Value: `🍔 מזון`
4. בפעם הבאה שעסק "קפה טסט" יופיע — הקטגוריה תיבחר אוטומטית
...
2. מצא את העסקה "קפה טסט" → מחק (החלק שמאלה)
3. עדכן `monthlyTotal` בחזרה ל-`0`
4. מחק `קפה טסט` מ-`merchantMap` (אם הוספת)
